...
winston.log('info', 'running healthcheck')
...
winston.log('debug', 'using GitHub user %s with token %s', GITHUB_USER, obfuscateString(GITHUB_TOKEN))
...
winston.log('debug', 'loading projects from %s', PROJECTS)
winston.log('debug', 'storing projects in %s', PROJECTS_ROOT_DIR)
const projects = require(PROJECTS)

const piDabProject = projects.find(project => project.name === 'mlenkeit/pi-dab')
if (!piDabProject) {
  winston.log('error', 'could not find configuration for mlenkeit/pi-dab project')
  process.exit(1)
}
...
getWebhookUrl({
  ...
  winston.log('debug', 'found webhook url %s', webhookUrl)
  return rp.get({
    uri: webhookUrl,
  ...
  if (response.statusCode === 200) {
    winston.log('info', 'healthcheck successful')
    return process.exit(0)
  }
  throw new Error(`Unexpected status code ${response.statusCode} instead of 200`)
}).catch(err => {
  winston.log('error', 'healthcheck failed: %s', err)
